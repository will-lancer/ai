...
class CheckpointRecoveryTests(unittest.TestCase):
    ...
    def test_valid_but_stale_latest_pointer_is_repaired_to_newest_checkpoint(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            store = CheckpointStore(temporary, "run-002-valid-stale", config_identity="cfg")
            first = store.save(4, {"step": 4})
            newest = store.save(7, {"step": 7})
            latest = store.run_dir / "latest.json"
            latest.write_text(
                json.dumps(
                    {
                        "schema_version": "lrh-checkpoint/v1",
                        "run_id": store.run_id,
                        "step": first.step,
                        "path": Path(first.path).name,
                        "metadata_sha256": first.metadata_sha256,
                        "config_identity": store.config_identity,
            ...
            self.assertEqual(recovered.step, newest.step)
            pointer = json.loads(latest.read_text(encoding="utf-8"))
            self.assertEqual(pointer["step"], newest.step)
            self.assertEqual(pointer["metadata_sha256"], newest.metadata_sha256)
