...
class CheckpointStore:
    ...
    def _ref_from_metadata(self, path: Path, metadata: Mapping[str, Any]) -> CheckpointRef:
        ...
        if marker_data.get("source_identity") != metadata.get("source_identity"):
            raise CheckpointError(f"completion marker source mismatch: {path}")
    ...
    def latest(self) -> CheckpointRef | None:
        """Return the newest valid checkpoint, repairing stale markers.

        ``latest.json`` is a cache.  It can be a syntactically valid pointer
        to an older completed checkpoint after a worker is interrupted between
        committing a checkpoint and refreshing the pointer.  Recovery scans
        every completed candidate and always chooses the greatest valid step.
        """

        latest_path = self.run_dir / LATEST_MARKER
        pointer_ref: CheckpointRef | None = None
        pointer_digest: str | None = None
        pointer_step: int | None = None
        try:
            pointer = _read_json(latest_path)
            if pointer.get("run_id") == self.run_id:
                candidate = (self.run_dir / str(pointer["path"])).resolve()
                ...
                checked = self._ref_from_metadata(candidate, metadata)
                pointer_step = int(pointer["step"])
                pointer_digest = str(pointer.get("metadata_sha256"))
                if checked.step == pointer_step and checked.metadata_sha256 == pointer_digest:
                    pointer_ref = checked
        except (CheckpointError, KeyError, OSError, TypeError, ValueError, json.JSONDecodeError):
            pointer_ref = None

        valid: list[CheckpointRef] = []
        for candidate in self._candidate_dirs():
            try:
                metadata = _read_json(candidate / "metadata.json")
                valid.append(self._ref_from_metadata(candidate, metadata))
            except (CheckpointError, KeyError, OSError, TypeError, ValueError, json.JSONDecodeError):
                continue
        if not valid:
            return None
        newest = max(valid, key=lambda ref: (int(ref.step), str(ref.path)))
        pointer_is_newest = (
            pointer_ref is not None
            and pointer_ref.step == newest.step
            and pointer_ref.metadata_sha256 == newest.metadata_sha256
            and pointer_step == newest.step
            and pointer_digest == newest.metadata_sha256
        )
        if not pointer_is_newest:
            atomic_write_json(
                latest_path,
                {
                    "schema_version": CHECKPOINT_SCHEMA_VERSION,
                    ...
                    "metadata_sha256": newest.metadata_sha256,
                    "config_identity": newest.config_identity,
                    "source_identity": newest.source_identity,
                },
            )
        return newest
    ...
    def _resolve_ref(self, ref: CheckpointRef | str | os.PathLike[str] | None) -> CheckpointRef:
        ...
        if isinstance(ref, CheckpointRef):
            ...
            checked = self._ref_from_metadata(path, metadata)
            if (
                checked.step != ref.step
                or checked.metadata_sha256 != ref.metadata_sha256
                or checked.config_identity != ref.config_identity
                or checked.source_identity != ref.source_identity
            ):
                raise CheckpointError("checkpoint reference identity does not match its artifact")
            return checked
        path = Path(ref)
    ...
    def is_run_complete(self) -> bool:
        ...
        try:
            ...
            return (
                ...
                and payload.get("source_identity") == self.source_identity
                and int(payload.get("step")) == reference.step
                and isinstance(payload.get("checkpoint"), Mapping)
                and payload["checkpoint"].get("metadata_sha256") == reference.metadata_sha256
            )
