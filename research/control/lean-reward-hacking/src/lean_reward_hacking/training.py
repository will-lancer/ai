    ...
    *,
    ...
    metric_values = torch.stack(
        (
            terms["loss"],
            terms["reward_loss"],
            terms["expected_reward"],
            terms["kl"],
            terms["l2"],
            terms["help_probability"],
            torch.as_tensor(grad_norm, device=terms["loss"].device),
        )
    ).detach().cpu().tolist()
...
def _new_state_from_resume(
    ...
    if isinstance(state.permutation, torch.Tensor):
        state.permutation = state.permutation.to(device="cpu")
    if state.sampler_state is not None:
        sampler_state = state.sampler_state
        if isinstance(sampler_state, torch.Tensor):
            sampler_state = sampler_state.to(device="cpu")
            state.sampler_state = sampler_state
        sampler_generator.set_state(sampler_state)  # type: ignore[arg-type]
    return state
...
def run_training(
    ...
    *,
    ...
    while state.global_step < target_steps:
        ...
        minibatch = _subset_batch(batch, indices, n_samples)
        metrics = train_step(
            agent,
            optimizer,
            minibatch,
