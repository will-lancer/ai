...
from .schemas import file_hash_records, load_table, sha256_file, validate_compact_bundle
...
def plot_all(
    ...
    *,
    ...
    basin = load_table(root, "basin_cells.csv")
    perturbations = load_table(root, "perturbation_trajectory.csv")
    control = load_table(root, "final_summary.csv") or checkpoints
    source_names = [
        name
        for name in (
            "runs.csv",
