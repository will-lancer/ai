...
class StatisticsConfig:
    ...
    alpha: float = 0.05
    minimum_component_weight: float = 0.20
    minimum_raw_c_off_separation: float = 0.30
...
_ALLOWED_TOP_LEVEL = frozenset(
    {
        ...
        "sensitivity_c_on_values",
        "sensitivity_invariant_c_off_values",
        "strategic_c_off_values",
        "optimizer",
        "grad_clip_norm",
        "harmful_goal_grid",
...
def _validate_mapping(raw: Mapping[str, Any]) -> None:
    ...
    if "model_kind" in raw:
        ...
        if model_kind not in {"factorized_mlp_v1", "quartic_route_v1"}:
            raise ConfigError("model_kind must be factorized_mlp_v1 or quartic_route_v1")
    ...
    for table_name, allowed in (("labels", {"c_on_min", "invariant_c_off_min", "strategic_c_off_max"}),
                                ("statistics", {"dip_bootstrap", "mixture_bootstrap", "bootstrap_seed", "alpha", "minimum_component_weight", "minimum_raw_c_off_separation", "bic_delta"})):
        if table_name not in raw:
            continue
        table = _require_mapping(raw[table_name], table_name)
        table_unknown = set(table) - allowed
    ...
    for key in ("route_scale", "initial_goal_route_amplitude", "initial_audit_sensitivity"):
        if key in raw and float(raw[key]) <= 0.0 and raw.get("model_kind") == "quartic_route_v1":
            raise ConfigError(f"{key} must be positive for quartic_route_v1")
    if "initial_route_noise" in raw and float(raw["initial_route_noise"]) < 0.0:
        raise ConfigError("initial_route_noise must be non-negative")
...
def _statistics_config(raw: Mapping[str, Any]) -> StatisticsConfig:
    ...
    for name in ("alpha", "minimum_component_weight", "minimum_raw_c_off_separation", "bic_delta"):
