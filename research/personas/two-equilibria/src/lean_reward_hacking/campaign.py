...
_DISTANCE_FIELDS = ("c_on", "c_off", "goal", "gate")
...
def _objective_record(values: Mapping[str, object]) -> dict[str, object]:
    reward = asdict(RewardConfig())
    reward_sha256 = _payload_sha256(reward)
    objective = {
        "version": "audited_expected_reward_uniform_kl_l2/v1",
        "training_audit_policy": "audit_on_only",
        "reward": reward,
        "reward_sha256": reward_sha256,
        "kl_to_uniform_coefficient": float(
            values.get("entropy_coefficient", values.get("kl_coefficient", 0.02))
        ),
        "l2_coefficient": float(values.get("weight_decay", 1.0e-4)),
        "optimizer": str(values.get("optimizer", "adam")).lower(),
    }
    return {
        "reward": reward,
        "reward_sha256": reward_sha256,
        "objective": objective,
        "objective_sha256": _payload_sha256(objective),
    }


def _write_experiment_identity(
    session_dir: Path,
    *,
    config_identity: str,
    ...
    record = {
        ...
        "source_commit": os.environ.get("RH_SOURCE_COMMIT", "unknown"),
        "train_dataset_sha256": train_hash,
        "eval_dataset_sha256": eval_hash,
        **_objective_record(values),
        **dict(extra or {}),
    }
    _write_json(session_dir / "spec" / "experiment_identity.json", record)
    return record
...
def _fields(name: str) -> tuple[str, ...]:
    preferred: dict[str, tuple[str, ...]] = {
        "runs.csv": (
            ...
            "final_label", "primary_step", "primary_label", "completed_step",
            "completed_label", "train_dataset_sha256", "eval_dataset_sha256",
            "reward_sha256", "objective_sha256", "source_archive_sha256",
            "threshold_set", "remote_raw_uri", "checkpoint_count", "retention_2T",
        ...
        "checkpoint_metrics.csv": (
            ...
            "hidden_misalignment", "train_loss", "n_pairs", "is_final", "is_primary",
            "is_terminal", "primary_step", "primary_label", "completed_step", "completed_label",
            "label",
        ),
        "final_summary.csv": (
            ...
            "primary_step", "primary_label", "completed_step", "completed_label",
        ...
        "perturbation_trajectory.csv": (
            ...
            "source_run_id", "source_step", "source_label", "branch_id",
            "branch_kind", "control_kind", "control_group", "optimizer_policy",
            "intervention", "strength", "branch_seed", "step_since_branch",
            "c_on", "c_off", "gap", "goal_score", "goal", "gate_effect", "gate", "off_audit_logit",
...
def _model(
    ...
    *,
    ...
    model_seed: int,
    device: str,
    model_kind: str = "factorized_mlp_v1",
    ...
    if model_kind == "quartic_route_v1":
        from .toy import initialize_quartic_agent

        return initialize_quartic_agent(
            task_dim=task_dim,
            harmful_goal_strength=harmful_goal_strength,
    ...
    if model_kind != "factorized_mlp_v1":
        raise CampaignError(f"unknown structured model_kind {model_kind!r}")
...
def _pair_count_row(
    ...
    *,
    ...
    """Count paired actions at the registered primary endpoint.

    ``step`` is deliberately retained as the measurement step.  The explicit
    endpoint fields make it impossible for a later continuation checkpoint to
    be mistaken for the row used by the primary statistical analysis.
    """

    if int(step) < 0:
        raise CampaignError("pair-count step must be non-negative")
    completed = int(step if completed_step is None else completed_step)
    if completed < int(step):
        raise CampaignError("completed_step cannot precede primary pair-count step")
...
def _step_from_row(row: Mapping[str, object], field: str = "step") -> int | None:
    """Read a non-negative integral checkpoint step."""
...
def _continuation_retention(
    ...
    *,
    ...
    """Check labels at the registered 2T and 4T checkpoints.

    The continuation check is descriptive.  A mode may disappear under
    continued training, which is evidence against the stability claim and
    remains visible in the compact run record.
    """
    ...
    for row in rows:
        ...
        if step is not None:
            by_step[step] = row
    expected = {
        "2T": int(primary_step) * 2,
        "4T": int(primary_step) * 4,
    }
    result: dict[str, object] = {
        ...
        "continuation_retention_status": "not_registered",
        "continuation_retention_ok": None,
    }
    checks: list[bool] = []
    for name, step in expected.items():
        ...
        if row is None:
            result["continuation_retention_status"] = "missing_checkpoint"
            result["continuation_retention_ok"] = False
            return result
        label = str(row.get("label") or row.get("mode_label") or Mode.INTERMEDIATE.value)
        retained = label == primary_label
        result[f"retention_{name}"] = retained
        result[f"continuation_{name}_label"] = label
        checks.append(retained)
    if checks:
        result["continuation_retention_status"] = "checked"
        result["continuation_retention_ok"] = all(checks)
    return result


def _annotate_endpoint_rows(
    rows: Sequence[Mapping[str, object]],
    *,
    primary_step: int,
    completed_step: int,
) -> tuple[list[dict[str, object]], dict[str, object]]:
    """Bind every checkpoint row to exact primary and completed endpoints."""

    normalized = [dict(row) for row in rows]
    primary_rows = [row for row in normalized if _step_from_row(row) == int(primary_step)]
    completed_rows = [row for row in normalized if _step_from_row(row) == int(completed_step)]
    if not primary_rows:
        raise CampaignError(f"no checkpoint row at registered primary step {primary_step}")
    if not completed_rows:
        raise CampaignError(f"no checkpoint row at completed step {completed_step}")
    primary_label = str(primary_rows[-1].get("label") or Mode.INTERMEDIATE.value)
    completed_label = str(completed_rows[-1].get("label") or Mode.INTERMEDIATE.value)
    retention = _continuation_retention(
        normalized,
    ...
    for row in normalized:
        step = _step_from_row(row)
        row["primary_step"] = int(primary_step)
        row["primary_label"] = primary_label
        row["completed_step"] = int(completed_step)
        row["completed_label"] = completed_label
        row["is_primary"] = step == int(primary_step)
        row["is_final"] = step == int(primary_step)
        row["is_terminal"] = step == int(completed_step)
    return normalized, {
...
def _run_one(
    *,
    ...
    if primary_step < 0 or primary_step > target_steps:
        raise CampaignError("primary_step must lie between zero and target_steps")
    ...
    agent = _model(
        ...
        model_seed=model_seed,
        device=device,
        model_kind=str(values.get("model_kind", "factorized_mlp_v1")),
        route_scale=float(values.get("route_scale", 8.0)),
        initial_goal_route_amplitude=float(
            values.get("initial_goal_route_amplitude", 0.10)
        ),
        initial_route_noise=float(values.get("initial_route_noise", 0.015)),
    )
    ...
    logged_steps = {int(row["step"]) for row in existing_rows if "step" in row}
    resume_state = None
    ...
    objective_identity = _objective_record(values)
    ...
    def checkpoint_callback(*, agent: object, optimizer: object, state: object, metrics: Mapping[str, float]) -> None:
        ...
        _save_training_checkpoint(
            ...
            metadata={
                ...
                "eval_dataset_sha256": eval_hash,
                "reward_sha256": objective_identity["reward_sha256"],
                "objective_sha256": objective_identity["objective_sha256"],
            },
        )
    ...
    def eval_callback(*, agent: object, state: object, metrics: Mapping[str, float]) -> None:
        ...
        row = _metric_row(
            ...
            is_final=step == primary_step,
            is_primary=step == primary_step,
            is_terminal=step == target_steps,
        )
    ...
    if resume_state is not None and int(resume_state.global_step) not in logged_steps:
        ...
        _append_jsonl(
            ...
            _metric_row(
                ...
                eval_hash=eval_hash,
                n_pairs=len(_opportunity_pairs(eval_pairs)),
                is_final=int(resume_state.global_step) == primary_step,
                is_primary=int(resume_state.global_step) == primary_step,
                is_terminal=int(resume_state.global_step) == target_steps,
            ),
        )
        logged_steps.add(int(resume_state.global_step))

    latest_before_fit = store.latest()
    run_reached_registered_completion = (
        store.is_run_complete()
        and latest_before_fit is not None
        and int(latest_before_fit.step) == int(target_steps)
    )
    if not run_reached_registered_completion:
        state = fit_replicate(
            agent,
            train_episodes,
            config,
    ...
    if not rows:
        latest_step = store.latest().step if store.latest() else target_steps
        evaluated = _evaluate(
            agent,
            eval_pairs,
            architecture=architecture,
        ...
        row = _metric_row(
            ...
            eval_hash=eval_hash,
            n_pairs=len(_opportunity_pairs(eval_pairs)),
            is_final=latest_step == primary_step,
            is_primary=latest_step == primary_step,
            is_terminal=latest_step == target_steps,
        )
        _append_jsonl(metrics_path, row)
        rows = [row]
    completed_step = int(target_steps)
    rows, endpoint_summary = _annotate_endpoint_rows(
        rows,
        primary_step=int(primary_step),
        completed_step=completed_step,
    )
    _write_jsonl(metrics_path, rows)
    primary_candidates = [row for row in rows if _step_from_row(row) == int(primary_step)]
    if not primary_candidates:
        raise CampaignError(
            f"run {run_id} has no evaluation row at registered primary step {primary_step}"
        )
    primary = dict(primary_candidates[-1])
    terminal = next(
        row for row in reversed(rows) if _step_from_row(row) == completed_step
    )
    primary["is_final"] = True
    primary["is_primary"] = True
    primary_label = str(primary.get("label", Mode.INTERMEDIATE.value))
    terminal_label = str(terminal.get("label", Mode.INTERMEDIATE.value))

    primary_ref = store.run_dir / f"checkpoint-{primary_step:08d}"
    try:
        primary_loaded = store.load(primary_ref, load_torch=True)
    except CheckpointError as exc:
        raise CampaignError(
            f"run {run_id} has no checkpoint at registered primary step {primary_step}"
        ) from exc
    primary_payload = primary_loaded.torch_state
    if not isinstance(primary_payload, Mapping) or not isinstance(
        primary_payload.get("model_state"), Mapping
    ):
        raise CampaignError(f"primary checkpoint lacks model state: {run_id}")
    agent.load_state_dict(primary_payload["model_state"])  # type: ignore[attr-defined]
    pair_counts = _pair_count_row(
        agent,
        eval_pairs,
        run_id,
        primary_step,
...
def _completion_valid(
    ...
    source_identity: str,
    *,
    require_table_checksums: bool = True,
) -> bool:
    try:
    ...
    try:
        marker_version = int(
            value.get("completion_schema_version", value.get("schema_version", 1))
        )
    except (TypeError, ValueError):
        return False
    ...
    if not strict:
        return True
    if marker_version >= _COMPLETION_MARKER_SCHEMA_VERSION:
        for field in (
            ...
            if not isinstance(digest, str) or len(digest) != 64:
                return False
        identity_path = path.parent.parent / "spec" / "experiment_identity.json"
        if (
            not identity_path.is_file()
            or sha256_file(identity_path) != value["experiment_identity_sha256"]
        ):
            return False
    ...
    except (CampaignError, OSError, UnicodeError, csv.Error):
        return False
...
def _audit_control_row(
    *,
    ...
    control_kind: str = "audit_feature_ablation",
) -> dict[str, object]:
...
def _generic_control_rows(
    *,
    ...
    """Record the three registered generic-network localization controls."""

    baseline_on = _finite_metric_value(baseline, "c_on")
    baseline_off = _finite_metric_value(baseline, "c_off")
    baseline_gap = _finite_metric_value(baseline, "gap")
    n_pairs = baseline.get("n_pairs", 0)
    available = all(value is not None for value in (baseline_on, baseline_off, baseline_gap))
    paired = {
        "available": available,
        "reason": "" if available else "paired baseline metrics unavailable",
        "c_on": baseline_on,
        "c_off": baseline_off,
        "gap": baseline_gap,
        "n_pairs": n_pairs,
    ...
    cue_swap = {
        "available": available,
        "reason": "" if available else "paired audit arms unavailable",
        "c_on": baseline_off,
        "c_off": baseline_on,
        "gap": None if baseline_gap is None else -baseline_gap,
        "n_pairs": n_pairs,
    }
    return [
        _audit_control_row(
        ...
        _audit_control_row(
            ...
            baseline=baseline,
            control=paired,
            control_kind="paired_input_coverage",
        ),
    ]
...
def _run_fixed(config: ExperimentConfig, remote_root: Path, session_dir: Path, config_identity: str) -> None:
    ...
    _write_experiment_identity(
        session_dir,
        config_identity=config_identity,
        values=values,
        train_hash=train_hash,
        eval_hash=eval_hash,
        extra={"experiment": experiment, "architecture": architecture},
    )
    ...
    for index in range(replicas):
        ...
        run_row, metrics, extras = _run_one(
            ...
            primary_step=int(values.get("steps", 20_000)),
        ...
        if architecture == "generic_mlp":
            ...
            tables["audit_control.csv"].extend(
                _generic_control_rows(
                    experiment=experiment,
                    architecture=architecture,
                    run_id=run_id,
                    baseline=baseline,
...
def _run_basin(config: ExperimentConfig, remote_root: Path, session_dir: Path, config_identity: str) -> None:
    ...
    train = make_training_episodes(
        int(values["train_episodes"]),
        task_dim,
        int(values["dataset_seed"]),
        float(values.get("opportunity_probability", 0.5)),
    )
    pairs = make_paired_evaluation(
        int(values["evaluation_pairs"]),
        task_dim,
        int(values["evaluation_seed"]),
        float(values.get("opportunity_probability", 0.5)),
    )
    train_hash = dataset_fingerprint(train)
    eval_hash = dataset_fingerprint(pairs)
    _write_experiment_identity(
        ...
        train_hash=train_hash,
        eval_hash=eval_hash,
        extra={"experiment": "toy_basin", "architecture": "toy"},
    )
    source_identity = _source_identity()
    tables: dict[str, list[dict[str, object]]] = {name: [] for name in TABLE_NAMES}
...
def _metric_scales(
    ...
    *,
    ...
    The population standard deviation is used for each field.  A field with
    one value or zero spread receives the registered finite floor, which keeps
    a constant source diagnostic from disappearing from the distance.  The
...
def _source_records(
    ...
    for path in sorted((remote_root / "runs" / "toy_fixed").glob("*/exports/checkpoint_metrics.csv")):
        ...
        run_by_id = {
            ...
            if item.get("run_id") and item.get("status") == "complete"
        }
        try:
            with path.open("r", encoding="utf-8", newline="") as handle:
...
def _branch_compact_row(
    ...
    *,
    ...
    experiment: str = "toy_perturbation",
    ...
    return {
        **row,
        ...
        "architecture": architecture,
        "config_sha256": config_identity,
        "source_config_sha256": source_config_identity,
        "source_archive_sha256": source_identity,
        ...
        "optimizer_policy": (
            "reset" if branch_kind == "reset_optimizer" else "preserve"
        ),
        "intervention": kind,
        "strength": strength,
        "branch_seed": branch_seed,
...
def _branch_status(
    ...
    *,
    ...
    minimum_monotone_fraction: float = 0.60,
    ...
    retention = (
        ...
        if requested_source
        and observed_labels
        and observed_labels[-1] == requested_source
        else 0.0
        if requested_source and observed_labels
        else None
    )
    ...
    step_values = [float(horizon(row)) for row in ordered]
    distance_values = [
        float(_finite_metric_value(row, "d_source"))
        for row in ordered
        if _finite_metric_value(row, "d_source") is not None
    ]
    slope = None
    nonincrease_fraction = None
    if len(distance_values) == len(step_values) and len(step_values) >= 2:
        transformed_steps = [math.log1p(max(0.0, value)) for value in step_values]
        mean_step = sum(transformed_steps) / len(transformed_steps)
        mean_distance = sum(distance_values) / len(distance_values)
        denominator = sum((value - mean_step) ** 2 for value in transformed_steps)
        if denominator > 0.0:
            slope = sum(
                (step - mean_step) * (distance - mean_distance)
                for step, distance in zip(
                    transformed_steps, distance_values, strict=True
                )
            ) / denominator
        nonincrease = [
            distance_values[index]
            <= distance_values[index - 1] + 1.0e-9
            for index in range(1, len(distance_values))
        ]
        nonincrease_fraction = sum(nonincrease) / len(nonincrease)
    if not intervention_feasible:
        ...
        recovery_ok = (
            ...
            and nonincrease_fraction >= minimum_monotone_fraction
        )
        if recovery_ok:
            status = "recovered"
    ...
    support = {
        ...
        "minimum_monotone_fraction": minimum_monotone_fraction,
        "source_distance_log_step_slope": slope,
    ...
    *,
    ...
    def new_agent_and_optimizer() -> tuple[object, object]:
        agent = _model(
            ...
            model_kind=str(base_values.get("model_kind", "factorized_mlp_v1")),
            route_scale=float(base_values.get("route_scale", 8.0)),
            initial_goal_route_amplitude=float(
                base_values.get("initial_goal_route_amplitude", 0.10)
            ),
            initial_route_noise=float(base_values.get("initial_route_noise", 0.015)),
        )
    ...
    if latest is not None:
        ...
        _save_training_checkpoint(
            ...
            metadata={
                ...
                "sampler_seed": int(source.get("shuffle_seed", 0)),
                "eval_dataset_sha256": eval_hash,
                "reward_sha256": _objective_record(base_values)["reward_sha256"],
                "objective_sha256": _objective_record(base_values)["objective_sha256"],
            },
        )
        current = 0
    ...
    def record(
        ...
        *,
        ...
        compact = _branch_compact_row(
            ...
            control_group=control_group,
            config_identity=config_identity,
            source_config_identity=toy_config_identity,
            source_identity=source_identity,
            train_dataset_hash=train_hash,
            eval_dataset_hash=eval_hash,
            reward_hash=str(_objective_record(base_values)["reward_sha256"]),
            objective_hash=str(_objective_record(base_values)["objective_sha256"]),
            intervention_diagnostics=intervention_diagnostics,
        )
...
def _run_perturbation(config: ExperimentConfig, remote_root: Path, session_dir: Path, config_identity: str) -> None:
    values = _standard_values(config)
    ...
    required_sources = int(values.get("sources_per_mode", 8))
    for label in (Mode.OVERSIGHT_INVARIANT.value, Mode.STRATEGIC.value):
        candidates = sorted(
        ...
        if len(candidates) < required_sources:
            raise CampaignError(
                f"perturbation campaign requires {required_sources} completed {label} "
                f"sources at step {source_step}; found {len(candidates)}"
            )
        selected.extend(candidates[:required_sources])
    ...
    _write_experiment_identity(
        ...
        eval_hash=eval_hash,
        extra={
            "experiment": "toy_perturbation",
            "source_config_sha256": toy_config_identity,
            "source_step": source_step,
            "perturbation_config_sha256": config_identity,
        },
    )
    tables: dict[str, list[dict[str, object]]] = {name: [] for name in TABLE_NAMES}
    ...
    preserve_tolerance = float(
        values.get(
            ...
            values.get("minimum_c_on_preservation_tolerance", 1.0e-3),
        )
    )
    ...
    for source_index, source in enumerate(selected):
        ...
        for intervention_index, (kind, strength) in enumerate(interventions):
            ...
            try:
                ...
                matched_controls = {
                    name: {**record.to_dict(), "run_branch_id": branch_ids.get(name, "")}
                    for name, record in registered_controls.items()
                }
            except (ImportError, TypeError, ValueError):
                matched_controls = {
                    name: {"run_branch_id": branch_ids.get(name, ""), "branch_kind": name}
            ...
            for branch_kind, control_kind, branch_intervention, branch_strength, branch_seed in branch_specs:
                ...
                branch_rows_current, support, _lineage = _run_perturbation_branch(
                    ...
                    minimum_monotone_fraction=float(values.get("minimum_monotone_fraction", 0.60)),
                    preserve_tolerance=preserve_tolerance,
                    target_tolerance=target_tolerance,
                ...
                lineage["matched_control_ids"] = dict(branch_ids)
                lineage["metric_scales"] = dict(scales)
                _write_json(lineage_path, lineage)
                if branch_kind == "frozen":
                    frozen_rows = branch_rows_current
                branch_rows.extend(branch_rows_current)
                _write_session_tables(
                    session_dir,
                    {"perturbation_trajectory.csv": branch_rows, **tables},
                )
...
def run_colab(config_path: str | Path, remote_root: str | Path) -> Path:
    ...
    marker = session_dir / "markers" / "completed.json"
    if _completion_valid(marker, identity, source):
        return session_dir
    ...
    _write_json(
        session_dir / "provenance" / "runtime.json",
        {
            "runtime": runtime_identity(
                ("numpy", "torch", "scipy", "diptest", "matplotlib", "psutil")
            ),
    ...
    experiment_identity_path = session_dir / "spec" / "experiment_identity.json"
    if not experiment_identity_path.is_file():
        raise CampaignError("campaign completed without an experiment identity record")
    experiment_identity = _read_json(experiment_identity_path)
    ...
    _write_json(
        marker,
        {
            ...
            "source_archive_sha256": source,
            "source_commit": experiment_identity.get("source_commit"),
            "train_dataset_sha256": experiment_identity.get("train_dataset_sha256"),
            "eval_dataset_sha256": experiment_identity.get("eval_dataset_sha256"),
            "reward_sha256": experiment_identity.get("reward_sha256"),
            "objective_sha256": experiment_identity.get("objective_sha256"),
            "experiment_identity_sha256": sha256_file(experiment_identity_path),
...
def tiny_validate(config_path: str | Path, remote_root: str | Path) -> dict[str, object]:
    ...
    try:
        ...
        agent = _model(
            ...
            harmful_goal_strength=float(values.get("harmful_goal_strength", 2.5)),
            initial_audit_sensitivity=float(values.get("initial_audit_sensitivity", 0.10)),
    ...
    for name in TABLE_NAMES:
        for path in sorted((remote_root / "runs").glob(f"*/*/exports/{name}")):
            ...
            if marker_payload.get("experiment") not in (None, path_experiment):
                continue
            complete_run_ids: set[str] | None = None
            if name != "runs.csv":
                runs_path = path.parent / "runs.csv"
                if runs_path.is_file():
                    try:
                        complete_run_ids = {
                            str(row.get("run_id"))
                            for row in _read_csv(runs_path)
                            if row.get("run_id") and row.get("status") in (None, "", "complete")
                        }
                    except (OSError, UnicodeError, csv.Error):
                        continue
            try:
...
def _refresh_compact_integrity(destination: Path) -> None:
    ...
    _write_text(destination / "checksums.sha256", make_checksums(destination))
...
def export_compact(remote_root: str | Path, local_bundle: str | Path) -> Path:
    ...
    manifest = {
        ...
        "provenance": {
            ...
            "sha256": sha256_file(destination / "provenance.json"),
        },
