...
class ConfigTests(unittest.TestCase):
    def test_repository_smoke_config_loads_with_defaults(self) -> None:
        ...
        self.assertEqual(config["model_kind"], "quartic_route_v1")
    ...
    def test_quartic_route_rejects_dead_initialization(self) -> None:
        with self.assertRaises(ConfigError):
            ExperimentConfig.from_mapping(
                {
                    "schema_version": 1,
                    "experiment": "unit",
                    "execution": "colab",
                    "model_kind": "quartic_route_v1",
                    "route_scale": 8.0,
                    "initial_goal_route_amplitude": 0.1,
                    "initial_audit_sensitivity": 0.0,
                    "initial_route_noise": 0.015,
                }
            )
